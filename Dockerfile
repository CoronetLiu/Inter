...
RUN mkdir  $HOME/.vnc
RUN echo 'coro' | vncpasswd -f > $HOME/.vnc/passwd
RUN echo '/bin/env  MOZ_FAKE_NO_SANDBOX=1  dbus-launch xfce4-session'  > $HOME/.vnc/xstartup
RUN chmod 600 $HOME/.vnc/passwd
RUN chmod 755 $HOME/.vnc/xstartup
RUN echo 'whoami ' >>/coro.sh
RUN echo 'cd ' >>/coro.sh
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/coro.sh
RUN echo 'cd /noVNC-1.2.0' >>/coro.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/coro.sh
RUN chmod 755 /coro.sh
EXPOSE 8900
CMD  /coro.sh
